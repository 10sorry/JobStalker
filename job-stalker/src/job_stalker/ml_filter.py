...
def get_filter_prompt(custom_prompt: str = "", resume_summary: str = "") -> str:
    ...
    base = f"""Ты фильтр вакансий. Проанализируй вакансию по критериям пользователя.
...
def get_comparison_prompt(vacancy_text: str, resume_text: str) -> str:
    ...
    return f"""Ты эксперт по IT-рекрутингу. Сравни вакансию с резюме и создай УЛУЧШЕННОЕ резюме.
...
ВАЖНО: В поле "improved_resume" напиши ПОЛНЫЙ текст улучшенного резюме (не краткий, а полноценный документ), адаптированный под эту вакансию. Добавь релевантные ключевые слова из вакансии, подчеркни подходящий опыт.
